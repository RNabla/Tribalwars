var HermitowskieFejkiPodstawka = {
    ...
    sanitizeCoordinates: function () {
        ...
        for (const coordinate of coordinates) {
            if (re.test(coordinate)) {
                if (this.checkConstraints(coordinate))
    ...
    countUnitsPopulation: function () {
        ...
        for (let i = 0; i < window.game_data.units.length; i++) {
            let unitName = window.game_data.units[i];
            let input = $(`#unit_input_${unitName}`);
            sum += Number(input.val() * this._unitsPopulation[unitName]);
        }
        return sum;
    },
    fill: function () {
    ...
    displayTargetInfo: function () {
        ...
        if (minutes < 10)
            minutes = '0' + minutes;
        UI.SuccessMessage(`Atak dojdzie ${when}na ${hour}:${minutes}`)
    },
