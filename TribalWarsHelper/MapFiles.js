...
function GetWorldInfo() {
    ...
    let TribalWarsHelper = {};
    return Promise.all([
        DownloadData(world, 'map/player.txt').then(text => {
            return Parser(text, ParsePlayer);
        }),
        DownloadData(world, 'map/ally.txt').then(text => {
            return Parser(text, ParseAlly);
        }),
        DownloadData(world, 'map/village.txt').then(text => {
            return Parser(text, ParseVillage);
        }),
        DownloadData(world, 'interface.php?func=get_config').then(text => {
            return Parser(text);
        }),
        DownloadData(world, 'interface.php?func=get_building_info').then(text => {
            return Parser(text);
        }),
        DownloadData(world, 'interface.php?func=get_unit_info').then(text => {
            return Parser(text);
        })

    ]).then(results => {
        TribalWarsHelper['player'] = results[0];
        TribalWarsHelper['ally'] = results[1];
        TribalWarsHelper['village'] = results[2];
        TribalWarsHelper['config'] = results[3];
        TribalWarsHelper['buildings'] = results[4];
        TribalWarsHelper['units'] = results[5];
        return TribalWarsHelper;
    }).catch(e => {
        Dialog.show('scriptError', `<h2>Blad podczas wykonywania skryptu</h2><p>Komunikat o bledzie: <br/><textarea>${e}</textarea></p>`)
    });

    function DownloadData(server, path) {
        let timestampKey = `${server}${path}${'timestamp'}`;
        let dataKey = `${server}${path}${'data'}`;
        let timestamp = localStorage[timestampKey];
        if (timestamp === undefined || (timestamp + 3600 * 1000) < Date.now()) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (data === undefined) {
                    ...
                    return DownloadData(server, path);
                }
                resolve(data);
            });
        }
    ...
    function ParsePlayer(rawLine) {
        ...
        return {
            id: Number(raw[0]),
            name: decodeURI(raw[1]).replace(new RegExp(/\+/, 'g'), ' '),
            allyId: Number(raw[2]),
    ...
    function ParseAlly(rawLine) {
        ...
        return {
            id: Number(raw[0]),
            name: decodeURI(raw[1].replace(new RegExp(/\+/, 'g'), ' ')),
            tag: decodeURI(raw[2].replace(new RegExp(/\+/, 'g'), ' ')),
