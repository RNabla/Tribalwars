    ...
    !function (TribalWars) {
        ...
        function get_bunker() {
            ...
            if (survivors) {
                ...
                if (summary.deff_population > 30000) {
                    info.bunker = summary;
                }
                else {
                    info.bunker = false;
        ...
        function generate_village_info() {
            ...
            if (info.bunker) {
                let url = Helper.generate_link_to_simulator(info.bunker.troops, { axe: 6000, light: 3000, ram: 400 }).substr(1);
        ...
        function parse_notebook(notes) {
            try {
                old_village_info = get_old_village_info(notes);
                attack_infos = get_attack_infos(notes);
                user_notes = get_user_notes(notes);
                ...
                if (old_attack_infos.some(old_info => old_info.report_id === info.report_id)) {
                ...
                attack_infos_text = attack_infos.map(x => generate_attack_info(x)).join("\n");
                let note_text = `${generate_village_info()}\n\n${attack_infos_text}\n<<<${info.player_id};TESTOWA>>>`;
                return note_text;
            }
            catch (e) {
                UI.ErrorMessage(e);
        ...
        function get_old_village_info(notes) {
            ...
            let player_id_match = notes.match(/<<<(\d+);/);
            if (player_id_match) {
                old_village_info.player_id = player_id_match[1];
            }
 ...
 * info.context
 ...
 info.units_away
 info.troops_type
 info.rebuild_time: {barracks, stable, garage}
 info.back_time
 info.battle_time
 info.attack_results
 info.church
 ...
 info.empty
 info.attack_results
 info.back_time // generowane jak powracają wojska z ataku, a my sie bronilismy
 info.rebuild_time //
 info.units_away //
 info.report_id //
 *
