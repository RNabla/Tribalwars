!function (TribalWars) {
    ...
    try {
        ...
        console.log('attack_info', attack_info);
        console.log('village_info', village_info);
        check_report();
    ...
    function check_report() {
        if (Object.keys(village_info).length === 1 &&
            Object.keys(attack_info).length === 3) {
    ...
    function get_context() {
        ...
        if (context.side !== 'att' && context.side !== 'def') {
            ...
            if (user_answer !== null && (user_answer.trim() === 'att' || user_answer.trim() === 'def')) {
                context.side = user_answer;
            } else {
                throw 'Nie wiem po której stronie dodać notatkę';
            }
        }
        context.opponent_side = context.side === 'att' ? 'def' : 'att';
        village_info.player_id = context.side === 'att' ? def_player_id : att_player_id;
        context.village_id = context.side === 'att' ? def_village_id : att_village_id;
        let village_player_id = (context.side === 'att' ? def : att)[0].rows[1].cells[1].children[0].getAttribute('data-player');
        if (village_info.player_id !== village_player_id) {
            // TODO: maybe clear old old_notes
            //setTimeout(() => {
            location.href = TribalWars.buildURL('GET', 'report', {
                action: 'del_one',
                mode: 'attack',
                id: attack_info.report_id,
                h: game_data.csrf
            });
            //}, 1000);
    ...
    function get_export_code() {
        attack_info.export_code = $("#report_export_code").val().match(/\[report_export].*\[\/report_export\]/)[0];
    }

    function get_report_id() {
        attack_info.report_id = Number(location.href.match(/view=(\d+)/)[1]);
    }
    ...
    function get_units_away() {
        ...
        if (spy_away.length === 1) {
            ...
            attack_info.units_away = summary;
        }
    }
    ...
    function get_troops_type() {
        ...
        if (attack_info.units_away) {
            let troops_type = verdict(attack_info.units_away, 1000);
            if (troops_type) {
                village_info.troops_type = troops_type;
                return;
            }
        }
        let attack_info_units = $(`#attack_info_${context.opponent_side}_units`)[0];
        if (attack_info_units) {
            let troops = get_troops_by_row(attack_info_units.rows[1], 1);
            if (troops) {
    ...
    function get_rebuild_time() {
        ...
        if (loses) {
            ...
            if (rebuild_time != 0) {
                attack_info.rebuild_time = new Date(attack_info.battle_time.getTime() + rebuild_time);
            }
        }
    }
    ...
    function get_back_time() {
        ...
        if (someone_survived) {
            ...
            let back_time_timestamp = attack_info.battle_time.getTime() + march_time * distance;
            ...
            attack_info.back_time = new Date(back_time_timestamp);
        }
    }
    ...
    function get_battle_time() {
        ...
        for (let i = 0; i < rows.length; i++) {
            if (rows[i].cells[0].innerText === "Czas bitwy") {
                attack_info.battle_time = Helper.parse_datetime_string(rows[i].cells[1].innerText);
                return;
            }
        }
    }
    ...
    function get_attack_results() {
        ...
        if (attack_results) {
            ...
            if (ram_match) {
                attack_info.attack_results.ram_result = ram_match.slice(1).map(x => Number(x));
            }
            if (catapult_match) {
                attack_info.attack_results.catapult_result = {
    ...
    function get_belief() {
        let attack_info = $(`#attack_info_${context.opponent_side}`);
        if (attack_info) {
            let belief_match = attack_info[0].innerText.match(/Siła uderzenia: (\d+)%/);
            if (belief_match) {
                village_info.belief = belief_match[1] === "100";
            }
        }
    }
    ...
    function generate_village_info() {
        let properties = [];
        if (village_info.troops_type) {
            properties.push(village_info.troops_type);
        }
        if (village_info.church) {
            properties.push(village_info.church);
        }
        if (typeof (village_info.belief) === 'boolean' && !village_info.belief) {
            properties.push('Bez wiary');
        }
        if (village_info.sim) {
            let url = Helper.generate_link_to_simulator(village_info.sim.troops, {
                axe: 6000,
                light: 3000,
                ram: 400
            }).substr(1);
            let deff_count = Helper.beautify_number(village_info.sim.deff_population);
            properties.push(`SIM [url=${url}]${deff_count}[/url]`);
        }
        if (village_info.sim_plaintext) {
            properties.push(village_info.sim_plaintext);
        }
        if (village_info.player_id) {
            properties.push(`[color=#F5EDDA]${village_info.player_id}[/color]`);
        }
    ...
    function append_note(new_note) {
        TribalWars.post('info_village', {
            ...
            id: context.village_id
        }, { note: new_note }, on_note_updated);
    }

    function on_note_updated() {
        UI.SuccessMessage(`Notatka dodana do wioski ${context.side === 'def' ? 'atakującego' : 'broniącego'}`);
        let next_report = $('#report-next')[0];
        if ($('#report-next')[0]) {
            location.href = next_report.href;
        }
    ...
    function get_current_notes() {
        let village_notes_url = TribalWars.buildURL('GET', { screen: 'info_village', id: context.village_id });
    ...
    function merge_village_info(old_village_info) {
        if (typeof (village_info.belief) === 'undefined') {
            village_info.belief = old_village_info.belief;
        }
        if (typeof (village_info.church) === 'undefined') {
            village_info.church = old_village_info.church;
        }
        if (typeof (village_info.troops_type) === 'undefined') {
            village_info.troops_type = old_village_info.troops_type;
        }
        if (typeof (village_info.sim) === 'undefined') {
            village_info.sim_plaintext = old_village_info.sim_plaintext;
        }
    }
    ...
    function parse_notebook(old_notes) {
        ...
        if (old_village_info.player_id !== village_info.player_id) {
            attack_infos = [];
        } else {
            merge_village_info(old_village_info);
        }
        // check if village already has this report
        if (attack_infos.some(old_info => old_info.report_id === attack_info.report_id)) {
            return { error: 'Village already has this report' };
        }
        // add current attack_info
        attack_infos.push(attack_info);
        // sort by date
