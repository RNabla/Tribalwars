...
import { handle_error, two_digit_number, get_timestamp_s } from './inf/Helper'
...
!(async function () {
    ...
    const Faking = {
        ...
        default_settings: {
            ...
            blocking_local: { time: 5, count: 1, block_players: true, scope: 'village' },
            blocking_global: [
                { time: 10, count: 1, name: 'global_1', block_players: false }
            ],
        ...
        load_config: async function (user_configuration) {
            ...
            for (const option_name in user_configuration) {
                if (!Faking.default_settings.hasOwnProperty(option_name)) {
            ...
            for (const option_name in Faking.default_settings) {
                if (user_configuration[option_name] != null) {
                    ...
                    Faking.settings[option_name] = Faking.default_settings[option_name];
                }
            }
        ...
        add_to_block_tables: async function (target) {
            if (Faking.settings.blocking_enabled) {
                await Faking.block_table_add_entry(
                    target,
                    Faking.blocking_local_get_key(),
                    Faking.settings.blocking_local.time
                );
        ...
        load_infrastructure: async function () {
            Faking.logger = await Logger.create_instance(namespace);
        ...
        main: async function (user_configuration) {
            Faking.logger.entry(arguments);
            Faking.check_screen();
    ...
    try {
        await Faking.load_infrastructure();
        await Faking.main(typeof (HermitowskieFejki) === "object" ? HermitowskieFejki : undefined);
    }
    catch (ex) {
        handle_error(ex, resources.FORUM_THREAD_HREF);
    }
})();
